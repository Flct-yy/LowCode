...
function Textarea({
  ...
  // 获得占位文本
  const placeholder = getConfigText(component.config, 'placeholder');

  // 获得行数
  const rows = getConfigValue<number>(component.config, 'rows') || 3;
  
  // 获得只读状态
  const readOnly = getConfigValue<boolean>(component.config, 'readOnly') || false;
  ...
  return (
    <div
      ref={divRef}
      className={`component-preview__default component__textarea ${newClassName}`}
      style={inlineStyle}
